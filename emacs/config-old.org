...
#+begin_src elisp
  (use-package projectile
      ...
      (leader-def 'normal
          ...
          "p c" 'projectile-run-async-shell-command-in-root
...
#+begin_src elisp
  ...
  (defun update-cwd (&rest args)
    (setq default-directory (eshell/pwd)))
  (advice-add 'eshell/cd :after #'update-cwd)
#+end_src
*** Aliases
#+begin_src elisp
...
#+begin_src elisp
  ...
  (add-hook 'vterm-mode-hook 'vterm-directory-sync)
...
#+begin_src elisp
  ...
  (defun my/kill-all-dired-buffers-and-quit ()
    ...
    (quit-window)
    (cd dired-directory)
  (dolist (buffer (buffer-list)) ; go through all buffers
      (with-current-buffer buffer ; switch to current buffer in loop
        (when (eq major-mode 'eshell-mode) ; check if it's eshell mode
